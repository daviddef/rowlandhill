...
def want_descent(sub):
    """Descend only into year-bearing subcats (or the 'by year' index)."""
    if SKIP_SUBCAT.search(sub):
        return False
...
def harvest_country(label, issuer_id):
    out, seen = [], set()
    stack = [f"Stamps of {label}", f"Postage stamps of {label}"]
    visited = set()
...
def main():
    cmap = json.load(open("country_issuer_map.json"))
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as ex:
        ...
        for fut in as_completed(futs):
            ...
            if done % 10 == 0 or done == len(items):   # checkpoint every 10 countries
                json.dump(all_recs, open("deep_records.json", "w"), ensure_ascii=False)
